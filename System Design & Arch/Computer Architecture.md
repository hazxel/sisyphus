...
指令级并行一般利用流水线和多指令发射等方式提高程序执行的并行度。

##### 流水线

指令流水线将每条指令分解为多步，每一步由独立的电路来执行，从而实现几条指令并行处理。每个时钟周期，每个流水线级都会产生各自的输出，存放于流水线寄存器（Pipeline Register）中，在下个时钟周期交由下一级处理。经典的五级流水线：

- 指令提取周期（IF）：送出 PC（程序计数器），将指令从存储器提取到指令寄存器（IR），将 PC 递增 4
- 指令译码/寄存器提取周期（ID）：指令译码，访问通用寄存器组（寄存器堆），读取操作数放入临时寄存器
- 执行/实际地址周期（EX）：不同指令所进行的操作不同，load/store 计算地址，alu 进行运算
- 寄存器访问/分支完成计算（MEM）：不同指令所进行的操作不同，load/store 会根据计算结果读取/写入。
- 写回周期（WB）：不同指令所进行的操作不同。load/alu 会把结果写入通用寄存器组。

现代 CPU 的流水线级可以达到 14 级，但每增加一级流水线级，就会多一次存取寄存器的操作，尽管这远小于流水线级本身的操作时间，但无限增加流水线的深度会导致这一过程的时间占比越来越大。其次，流水线深度的增加还会导致冒险问题更难解决。

##### 超标量/多指令发射

在每个时钟周期发出多条指令，并在每一级流水线中并行执行，可进一步提高流水线性能。 多发射处理器可以分为两类：VLIW 和超标量。 两者的区别在于：VLIW 处理器由编译器静态决定指令调度，而超标量处理器是由硬件动态调度指令。

超标量处理器可以进一步分为两类：顺序发射（in-order issue)和乱序发射(out-of-order issue)。
