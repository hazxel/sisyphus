...
每个流水线级的输出都要放到流水线寄存器（Pipeline Register）中，交由下一级流水线级进行处理。所以每增加一级流水线级，就会多一次存取寄存器的操作，尽管相比流水线级本身的操作时间要快的多，但无限增加流水线的深度会导致这一过程的时间占比越来越大。其次，流水线深度的增加还会导致冒险问题更难解决。
